...
function Game() {
  ...
  function onSubmit(event) {
    ...
    const updatedGuessList = [...guessList, {
                                              ...
                                              statusArray: checkGuess(guess, answer)
                                            }];
    setGuessList(updatedGuessList);

    // update gameStatus
    if( guess === answer) { setGameStatus('win'); }
    else if( updatedGuessList.length == NUM_OF_GUESSES_ALLOWED ) { setGameStatus('lose') }
          ...
          { !guessList[index] ?
            ...
            <>
              {guessList[index].label.split("").map((letter, letterIndex) => (
                <span className={`cell ${guessList[index].statusArray[letterIndex].status}`} id={`guess-cell-${index+1}-${letterIndex+1}`} key={letterIndex+1}>
                  {letter}
                </span>
              ))}
            </>
